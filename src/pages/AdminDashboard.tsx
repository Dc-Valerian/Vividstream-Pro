...
const AdminDashboard = () => {
  ...
  // Modal states
  const [viewModal, setViewModal] = useState<{ open: boolean; title: string; data: Record<string, any> | null }>({
    open: false,
    title: "",
    data: null,
  });
  const [editModal, setEditModal] = useState<{ open: boolean; title: string; data: Record<string, any> | null; type: string }>({
    open: false,
    title: "",
    data: null,
    type: "",
  });
  const [deleteModal, setDeleteModal] = useState<{ open: boolean; title: string; id: string | number | null; type: string }>({
    open: false,
    title: "",
    id: null,
    type: "",
  ...
  const [tickets, setTickets] = useState([
    { id: "TKT-001", user: "John Doe", code: "WIN-2024-X", status: "Redeemed", date: "2024-03-15" },
    { id: "TKT-002", user: "Jane Smith", code: "WIN-2024-Y", status: "Pending", date: "2024-03-14" },
    { id: "TKT-003", user: "Mike Johnson", code: "WIN-2024-Z", status: "Expired", date: "2024-03-13" },
  ]);

  const [users, setUsers] = useState([
    { id: 1, name: "John Doe", email: "john@example.com", status: "Active", wallet: "2,500" },
    { id: 2, name: "Jane Smith", email: "jane@example.com", status: "Active", wallet: "1,200" },
    { id: 3, name: "Mike Johnson", email: "mike@example.com", status: "Suspended", wallet: "0" },
  ]);

  const [bets, setBets] = useState([
    { id: "BET-001", user: "John Doe", match: "USA vs Brazil", prediction: "USA", stake: "500", status: "Open" },
    { id: "BET-002", user: "Jane Smith", match: "Germany vs France", prediction: "Draw", stake: "300", status: "Won" },
    { id: "BET-003", user: "Mike Johnson", match: "Spain vs Italy", prediction: "Spain", stake: "200", status: "Lost" },
  ]);

  const [visaApplications, setVisaApplications] = useState([
    { id: "VISA-001", user: "John Doe", destination: "USA", status: "Under Review", submitted: "2024-03-10" },
    { id: "VISA-002", user: "Jane Smith", destination: "UK", status: "Approved", submitted: "2024-03-08" },
    { id: "VISA-003", user: "Mike Johnson", destination: "France", status: "Pending Documents", submitted: "2024-03-12" },
  ]);

  const [payments, setPayments] = useState([
    { id: "PAY-001", user: "John Doe", amount: "$250", method: "Crypto", status: "Completed", date: "2024-03-15" },
    { id: "PAY-002", user: "Jane Smith", amount: "$150", method: "Card", status: "Completed", date: "2024-03-14" },
    { id: "PAY-003", user: "Mike Johnson", amount: "$300", method: "Crypto", status: "Pending", date: "2024-03-13" },
  ]);
  ...
  const stats = [
    ...
    { title: "Tickets Redeemed", value: "3,421", change: "+8%", trend: "up", icon: Ticket },
    { title: "Visa Applications", value: "892", change: "+23%", trend: "up", icon: Plane },
    { title: "Revenue", value: "$128,450", change: "-3%", trend: "down", icon: CreditCard },
  ];
  ...
  // View handlers
  const handleView = (title: string, data: Record<string, any>) => {
    setViewModal({ open: true, title, data });
  };

  // Edit handlers
  const handleEdit = (title: string, data: Record<string, any>, type: string) => {
  ...
  const handleSaveEdit = (updatedData: Record<string, any>) => {
    switch (editModal.type) {
      case "ticket":
        setTickets(prev => prev.map(t => t.id === updatedData.id ? updatedData as typeof t : t));
        break;
      case "user":
        setUsers(prev => prev.map(u => u.id === updatedData.id ? updatedData as typeof u : u));
        break;
      case "bet":
        setBets(prev => prev.map(b => b.id === updatedData.id ? updatedData as typeof b : b));
        break;
      case "visa":
        setVisaApplications(prev => prev.map(v => v.id === updatedData.id ? updatedData as typeof v : v));
        break;
      case "payment":
        setPayments(prev => prev.map(p => p.id === updatedData.id ? updatedData as typeof p : p));
        break;
    }
  };
            ...
            {sidebarLinks.map((link) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-colors ${
                  (link.tab === activeTab) || (link.id === "analytics" && !activeTab)
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            <TabsList className="bg-secondary/50 p-1 rounded-xl">
              ...
              <TabsTrigger value="betting" className="rounded-lg">Betting</TabsTrigger>
            ...
            <TabsContent value="tickets">
              <div className="rounded-2xl border border-border bg-card overflow-hidden">
                <div className="p-4 border-b border-border flex items-center justify-between">
                  ...
                  <Button size="sm" variant="gradient">Add Ticket</Button>
                </div>
                          ...
                          <td className="p-4">
                            <div className="flex gap-2">
                              <Button 
                                ...
                                variant="ghost" 
                                className="h-8 w-8"
                                onClick={() => handleView("Ticket Details", ticket)}
                              >
                              ...
                              <Button 
                                ...
                                variant="ghost" 
                                className="h-8 w-8"
                                onClick={() => handleEdit("Ticket", ticket, "ticket")}
                              >
                              ...
                              <Button 
                                ...
                                onClick={() => handleDelete("Ticket", ticket.id, "ticket")}
                              >
            ...
            <TabsContent value="users">
              <div className="rounded-2xl border border-border bg-card overflow-hidden">
                <div className="p-4 border-b border-border flex items-center justify-between">
                  ...
                  <Button size="sm" variant="gradient">Add User</Button>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-secondary/50">
                    ...
                    <tbody>
                      {users.map((user) => (
                        <tr key={user.id} className="border-t border-border hover:bg-secondary/30 transition-colors">
                          ...
                          <td className="p-4 font-medium text-primary">{user.wallet}</td>
                          <td className="p-4">
                            <div className="flex gap-2">
                              <Button 
                                size="icon" 
                              ...
                              <Button 
                                ...
                                variant="ghost" 
                                className="h-8 w-8 text-destructive"
                                onClick={() => handleDelete("User", user.id, "user")}
                              >
                                <Trash2 className="w-4 h-4" />
                              </Button>
                            </div>
            ...
            <TabsContent value="betting">
              <div className="rounded-2xl border border-border bg-card overflow-hidden">
                <div className="p-4 border-b border-border flex items-center justify-between">
                  ...
                  <Button size="sm" variant="gradient">Add Match</Button>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-secondary/50">
                      <tr>
                        ...
                        <th className="text-left p-4 text-sm font-medium text-muted-foreground">Stake</th>
                        <th className="text-left p-4 text-sm font-medium text-muted-foreground">Status</th>
                        <th className="text-left p-4 text-sm font-medium text-muted-foreground">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {bets.map((bet) => (
                        <tr key={bet.id} className="border-t border-border hover:bg-secondary/30 transition-colors">
                          ...
                          <td className="p-4 font-medium text-primary">{bet.stake}</td>
                          <td className="p-4">
            ...
            <TabsContent value="visa">
              <div className="rounded-2xl border border-border bg-card overflow-hidden">
                <div className="p-4 border-b border-border flex items-center justify-between">
                  ...
                  <Button size="sm" variant="gradient">Export</Button>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-secondary/50">
                    ...
                    <tbody>
                      {visaApplications.map((visa) => (
                        <tr key={visa.id} className="border-t border-border hover:bg-secondary/30 transition-colors">
                          <td className="p-4 font-mono text-sm">{visa.id}</td>
                          <td className="p-4">{visa.user}</td>
                          <td className="p-4">{visa.destination}</td>
                          <td className="p-4">
                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusBadge(visa.status)}`}>
                              {visa.status}
                            </span>
                          </td>
                          <td className="p-4 text-muted-foreground">{visa.submitted}</td>
                          <td className="p-4">
                            <div className="flex gap-2">
                              <Button 
                                size="icon" 
                                variant="ghost" 
                                className="h-8 w-8"
                                onClick={() => handleView("Visa Application Details", visa)}
                              >
                                <Eye className="w-4 h-4" />
                              </Button>
                              <Button 
                                ...
                                variant="ghost" 
                                className="h-8 w-8"
                                onClick={() => handleEdit("Visa Application", visa, "visa")}
                              >
                                <Edit className="w-4 h-4" />
                              </Button>
                            </div>
                    ...
                    <tbody>
                      {payments.map((payment) => (
                        <tr key={payment.id} className="border-t border-border hover:bg-secondary/30 transition-colors">
                          ...
                          <td className="p-4 font-medium text-primary">{payment.amount}</td>
                          ...
                          <td className="p-4">
                            <div className="flex gap-2">
                              <Button 
                                ...
                                variant="ghost" 
                                className="h-8 w-8"
                                onClick={() => handleView("Payment Details", payment)}
                              >
                                <Eye className="w-4 h-4" />
                              </Button>
                            </div>
